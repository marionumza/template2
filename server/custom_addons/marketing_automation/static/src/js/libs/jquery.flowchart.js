...
// "colorize" the widget name
    $.widget("flowchart.flowchart", {
        ...
        options: {
            ...
            onOperatorMouseOver: function (operatorId, $el) {
                return true;
            },
            onOperatorMouseOut: function (operatorId, $el) {
                return true;
            },
        ...
        _initEvents: function () {
            ...
            this.element.on('mouseup click',function (e) {
                var $this = $(this);
                var offset = $this.offset();
            ...
            this.objs.layers.operators.on('mousedown mouseup', '.flowchart-operator-connector', function () {
                var $this = $(this);
            ...
            this.objs.layers.operators.on('mouseenter', '.flowchart-operator', function (e) {
                self._operatorMouseOver($(this).data('operator_id'), $(this));
            });

            this.objs.layers.operators.on('mouseleave', '.flowchart-operator', function (e) {
                self._operatorMouseOut($(this).data('operator_id'), $(this));
            });
        ...
        _drawLink: function (linkId) {
            ...
            var link_text = linkData['link_text']//fromOperator['properties']['outputs'][linkData.fromConnector]['link_text'];
            ...
            shape_text.setAttribute('text-anchor', 'middle');
            shape_text.textContent = link_text;
            group.appendChild(shape_text);
            linkData.internal.els.text = shape_text;

            var rect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
            var SVGRect = shape_text.getBBox();
            rect.setAttribute("x", SVGRect.x);
            rect.setAttribute("y", SVGRect.y);
            rect.setAttribute("width", SVGRect.width);
            rect.setAttribute("height", SVGRect.height);
            rect.setAttribute("fill", "#ffffff");
            group.insertBefore(rect, shape_text);
            linkData.internal.els.text_rect = rect;
        ...
        _refreshLinkPositions: function (linkId) {
            ...
            var linkTextX = ((bezierFromX / 2) + (bezierToX / 2));
            var linkTextY = ((fromY / 2) + (toY / 2));
            ...
            linkData.internal.els.text.setAttribute("x", linkTextX);
            linkData.internal.els.text.setAttribute("y", linkTextY);
            linkData.internal.els.text_rect.setAttribute("x", linkData.internal.els.text.getBBox().x);
            linkData.internal.els.text_rect.setAttribute("y", linkData.internal.els.text.getBBox().y);
        ...
        _getOperatorFullElement: function (operatorData) {
            ...
            $operator.addClass(infos.custom_class);
            var $operator_title = $('<div class="flowchart-operator-title"></div>');
            var $operator_icon = $('<i class="' + infos.fa_icon + '"/> <span>' + infos.title + '</span>');
            $operator_title.html($operator_icon);
        ...
        createOperator: function (operatorId, operatorData) {
            ...
            var height = fullElement.operator.find('.flowchart-operator-title').prop('offsetHeight');
            fullElement.operator.css({top: operatorData.top, left: operatorData.left, height: height, minHeight: '100px'});
        ...
        _operatorMouseOver: function (operatorId, $el) {
            if (!this.callbackEvent('operatorMouseOver', [operatorId, $el])) {
                return;
            }
            this._addHoverClassOperator(operatorId);
        },

        _operatorMouseOut: function (operatorId, $el) {
            if (!this.callbackEvent('operatorMouseOut', [operatorId, $el])) {
                return;
            }
