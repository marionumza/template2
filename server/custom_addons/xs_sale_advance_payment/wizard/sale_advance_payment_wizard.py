...
class AccountVoucherWizard(models.TransientModel):
    ...
    def check_amount(self):
        ...
        if self.env.context.get('active_id', False):
            ...
            if str(self.amount_advance) > ("%.2f" % round(order.amount_residual,2)):
    ...
    def default_get(self, fields):
        ...
        if 'amount_total' in fields:
            res.update({'amount_total': sale.amount_residual, 'currency_id': sale.pricelist_id.currency_id.id})
    ...
    def onchange_date(self):
        if self.currency_id:
            currency = self.currency_id or (self.journal_id.currency_id or self.env.user.company_id.currency_id)
            self.exchange_rate = 1.0 / (self.env["res.currency"].with_context(date=self.date)._get_conversion_rate(currency, self.env.user.company_id.currency_id) or 1.0)
