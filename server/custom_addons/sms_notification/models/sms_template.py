...
class SmsTemplate(models.Model):
    ...
    def get_body_data(self, obj, partner_id=None):
        self.ensure_one()
        if obj:
            body_msg = self.env["mail.template"].with_context(lang=partner_id.lang if partner_id else '').sudo().render_template(
                self.with_context(lang=partner_id.lang if partner_id else '').sms_body_html, self.model, [obj.id])
