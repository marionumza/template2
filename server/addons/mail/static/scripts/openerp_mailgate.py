...
def configure_parser():
    ...
    group = optparse.OptionGroup(parser, "Note",
        ...
        "with the Multitouchpro server for case management in the CRM module.")
    parser.add_option_group(group)
    parser.add_option("-u", "--user", dest="userid",
                      help="Multitouchpro user id to connect with",
                      default=config.OPENERP_DEFAULT_USER_ID, type='int')
    parser.add_option("-p", "--password", dest="password",
                      help="Multitouchpro user password",
    ...
    parser.add_option("-d", "--dbname", dest="dbname",
                      help="Multitouchpro database name (default: %default)",
                      default=config.OPENERP_DEFAULT_DATABASE)
    parser.add_option("--host", dest="host",
                      help="Multitouchpro Server hostname",
                      default=config.OPENERP_HOSTNAME)
    parser.add_option("--port", dest="port",
                      help="Multitouchpro Server XML-RPC port number",
...
def main():
    ...
    try:
        ...
        subject = '[Multitouchpro]:ERROR: Mailgateway - %s' % time.strftime('%Y-%m-%d %H:%M:%S')
        send_mail(
            config.MAIL_ERROR,
            config.MAIL_ADMINS,
            subject, msg, files=[('message.txt', msg_txt)]
        )
        sys.stderr.write("Failed to deliver email to Multitouchpro Server, sending error notification to %s\n" % config.MAIL_ADMINS)
