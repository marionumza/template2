...
QUnit.module('fields', {
    ...
    QUnit.test('Sms widgets are correctly rendered', function (assert) {
        ...
        // GSM-7
        form.$('.o_input').val("Hello from Multitouchpro").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '15 / 1 SMS (GSM7) ', 'Should be "15 / 1 SMS (GSM7) " for "Hello from Multitouchpro"');
        // GSM-7 with \n => this one count as 2 characters
        form.$('.o_input').val("Hello from Multitouchpro\n").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '17 / 1 SMS (GSM7) ', 'Should be "17 / 1 SMS (GSM7) " for "Hello from Multitouchpro\\n"');
        // Unicode => ê
        form.$('.o_input').val("Hêllo from Multitouchpro").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '15 / 1 SMS (UNICODE) ', 'Should be "15 / 1 SMS (UNICODE) " for "Hêllo from Multitouchpro"');
        // GSM-7 with 160c
